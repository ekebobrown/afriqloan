    ...
    <>
        <section id="form" className={`${styles.fh1} d-flex justify-content-center mx-auto text-primary px-4`}>
          <div className={`container-md d-flex flex-column justify-content-around bg-white rounded-3 p-4 p-md-5 my-5 row-gap-3 align-self-center`} style={{maxWidth:"600px"}}>
            ...
            <small id="info" className="flex-fill text-center overflow-hidden" style={{height:activation||redirect?'32px':'0px', transition:'height 0.5s ease-in-out'}}>
              {(activation||redirect) && <div className={`d-flex p-1 ${activation?'bg-success-subtle':'bg-warning-subtle'} rounded-2 justify-content-center align-items-center w-100`}><i className="fa-solid fa-circle-info me-2"></i>{activation?"Account successfully activated":"Invalid or expired session information"}</div>}
            </small>
            {!activation &&
