...
export async function GET(request){
    ...
    try{
        if(!isAuthenticated) throw new Error("You are not authorized to access this resource. Invalid session information!")
        ...
        return NextResponse.json({success:true, message:"Operation successful.", count:orders[0]?.count||0, lists:orders[0]?.lists||[]})
    }catch(error){
