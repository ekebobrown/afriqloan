...
export default function Recent({userId}){
    ...
    return (
        <>
            <div className="d-flex flex-column flex-md-row p-4 rounded-3 gap-2 align-items-center">
                ...
                {orders.message?<button className="align-self-stretch d-flex rounded-2 border border-1 justify-content-between align-items-center px-4 py-2 bg-body position-relative" disabled={!orders.success||orders.status==='loading'}>
                    <input type="date" min="2024-08-01" max={`${new Date().toISOString().split('T')[0]}`} className="position-absolute top-0 start-0 w-100 opacity-0 py-2"  value={date} onChange={(e)=>setDate(e.target.value)} disabled={!orders.success||orders.status==='loading'} />
                    {date||'Select Date'}
                ...
                {orders.message?
                    <button className="dropdown-toggle align-self-stretch d-flex rounded-2 border border-1 justify-content-between align-items-center px-4 py-2 bg-body position-relative" disabled={!orders.success||orders.status==='loading'} data-bs-toggle="dropdown" aria-expanded="false">
                        Filters
                ...
                {orders.message?
                    <button className={`align-self-stretch d-flex rounded-2 border border-1 align-items-center px-3 py-2`} style={{backgroundColor:'rgba(137,70,207,0.2)', color:'#8946CF'}} onClick={()=>{setFilter(orders.lists); JSON.stringify(initialSort)!==JSON.stringify(sort) && setSort(initialSort); date!==""&&setDate(''); window.history.pushState({}, '', '/listings/orders')}} disabled={!orders.success||orders.status==='loading'}>Reset All</button>:
                    <Skeleton width={80} className="btn rounded-2 border border-1 px-4 py-3" highlightColor="#cbaee6" baseColor="#DFCCF1" direction="rtl" duration={2} />
                }
            </div>
                    ...
                    <table className="table table-hover table-responsive fs-6 text-nowrap mb-0">
                        <thead>
                            <tr className="position-relative">
                                <th className="" onClick={()=>setSort({column:"item.title", order:sort.order===1?-1:1})}>Product</th>
                                <th className="" onClick={()=>setSort({column:"contact.email", order:sort.order===1?-1:1})}>Customer</th>
                                <th className="" onClick={()=>setSort({column:"value", order:sort.order===1?-1:1})}>Total</th>
                                <th className="" onClick={()=>setSort({column:"status", order:sort.order===1?-1:1})}>Status</th>
                                ...
                                <td>
                                    <div className="d-flex flex-row align-items-center gap-1">
                                        {order.item?.image?<Image
                                            src={order.item?.image[Math.floor(Math.random()*order.item?.image.length)]}
                                            height={40}
                                            width={40}
                                        ...
                                        <div className="d-flex flex-column">
                                            {order.item?.title||<Skeleton width={100} />}{order?.quantity?order.quantity>1 && <span>+{order?.quantity-1} other items</span>:<Skeleton width={80} />}
                                        </div>
                                    </div></td>
