...
export function User({ user }) {
    ...
    const {data} = useFetcher('api/messages')
    const newmessages = data?.filter((message)=>message.flags.status==="unread").length
    ...
    return (
        <>
            ...
            {user?.loans &&
                <>
                    <li className={`${pathname.includes('loans')?'active':''}`}><Link href="/loans" className="link d-inline-flex align-items-center"><span className="fa-solid fa-landmark fa-fw me-2"></span>Loans</Link></li>
            ...
            <>
                <div className="position-relative">
                    ...
                    {newmessages>0&&<div className="position-absolute d-flex align-items-center justify-content-center rounded-circle bg-warning" style={{width:'18px', height:'18px', top:'2px', left:'12px'}}><small>{newmessages}</small></div>}
                </div>
